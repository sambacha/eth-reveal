...
const tryAddABIAndMaybeProxyTarget = async ({
  ...
  if (!abiCache[contract]) {
    abiCache[contract] = tryGetABIAndSource({
      contract,
      network,
      etherscanKey,
    });
  ...
  if (contractInstance.target) {
    ...
    if (!abiCache[target]) {
      abiCache[target] = tryGetABIAndSource({
        contract: target,
        network,
        etherscanKey,
      });
...
const lookup = async ({
  ...
  etherscanKey = '6XENF1GSDZYRISER6X4KGF3VDNNFPI47H4',
  ...
  if (isContract) {
    ...
    decodedLogs = addReadableValues(abiDecoder.decodeLogs(logs)).map(
      (log, i) => log || logs[i],
    );
  }
